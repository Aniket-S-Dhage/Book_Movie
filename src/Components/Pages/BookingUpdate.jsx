...
const BookingUpdate = () => {
    ...
    const getData = async()=>{
        ...
        const response = await axios.get(`http://localhost:8020/bookings/${bookingID}`)

        const result = response.data

        setValue('name', result.name)
        setValue('mob', result.mob)
        setValue('gender', result.gender)
        setValue('movie', result.movie)   
        setValue('date', result.date)   
        setValue('time', result.time)
 
        const new_seats = [...seats]
        for(let arr of result.bseats){
            let row = arr[0]
            let col = arr[1]

            new_seats[row][col] = 'booked'
        }
    ...
    return (
        <div className='d-flex justify-content-center mt-5'>
            <form className='form-control w-75' onSubmit={handleSubmit(updateData)}>
